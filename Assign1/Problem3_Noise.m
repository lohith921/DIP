...
h5 = ones(5,5)/25;
% Creating the gaussian filter with hsize = [3 3] and sigma = 2
Gauss3 = fspecial('gaussian',[3 3],2);
% Creating the gaussian filter with hsize = [5 5] and sigma = 2
...
figure, imshow(I1); title('Over exposed image with Gaussian Noise');

% filtering using 3x3 filter
I2 = imfilter(I1,h); 
figure, imshow(I2); title('Over exposed image with 3x3 Average filter');

% filtering using 5x5 filter
I3 = imfilter(I1,h5); 
figure, imshow(I3); title('Over exposed image with 5x5 Average filter');

% filtering using 3x3 Gaussian filter
I4 = imfilter(I1,Gauss1,'same');
figure, imshow(I4); title('Over exposed image with 3x3 Gauss filter');
